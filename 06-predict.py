"""Predict per-pixel on a set of hls tile/year combos.

Given:
* a model file (--model-path)
* model configuration files (--sources-path, --params-path)
* csv of tile/year combinations to predict on (--tiles-path)
* regression prediction scalars (-s)
* output directory (-o)

Produces a single band COG for each tile/year/prediction variable combination stored at:
{output_dir}/{variable}/{year}/{tile}.tif

Each COG is the same dimension as its corresponding HLS tile with the same geospatial metadata (transform, crs)
...
from src.prediction_dataset import DatasetGenerator
    ...
    for tile, attrs, ds in ds_generator:
        ...
        try:
            for i, ((xs, ys), inputs) in enumerate(dl):
                ...
                output_tile[ys, xs, :] = torch.cat([class_pred, reg_pred * scales_tensor], dim=1).cpu().float()
...
if __name__ == "__main__":
    ...
    parser.add_argument('-s', '--scales', nargs='+', help='Scale factors for each regression variable. Printed by the training script.')
    ...
    with open(args.sources_path) as f:
        sources = yaml.load(f)
    #Set default blob vars
    if sources.get('storage_account') == "":
        sources['storage_account'] = os.environ['STRG_ACCOUNT_NAME']
    if sources.get('account_key') == "":
        sources['account_key'] = os.environ['STRG_ACCOUNT_KEY']
    print(sources)
    ...
    ds_gen = DatasetGenerator(
        ...
        use_naip=len(sources.get('naip_path')) is not 0,
        naip_size=params.get('naip_size'),
        use_hls=len(sources.get('hls_path')) is not 0,
        hls_size=params.get('hls_size'),
        checkpoints=checkpoints,
    )
    args.scales = np.array(args.scales).astype(float)
    predict(
        args.model_path,
        args.tiles_path,
        ds_gen,
